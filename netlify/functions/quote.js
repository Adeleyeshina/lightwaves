...
exports.handler = async (event) => {
  ...
  let body;
  try { body = JSON.parse(event.body); } catch { return { statusCode: 400, body: JSON.stringify({ message: 'Invalid JSON.' }) }; }

  const { fullName, email, phone, location, service, description } = body || {};
  ...
  if (!name || !mail || !phoneNum) {
    return { statusCode: 400, body: JSON.stringify({ message: 'Full name, email, and phone are required.' }) };
  }
  if (!isValidEmail(mail)) {
    return { statusCode: 400, body: JSON.stringify({ message: 'Please provide a valid email address.' }) };
  }
  ...
  try {
    ...
    return { statusCode: 200, body: JSON.stringify({ message: 'Quote request submitted successfully.' }) };
  } catch (err) {
    console.error('Admin email error:', err);
    return { statusCode: 500, body: JSON.stringify({ message: 'Failed to send email. Please try again or contact us via WhatsApp.' }) };
  }
};
